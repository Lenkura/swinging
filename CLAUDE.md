...
**`blades`** — `[{ x, y, w, h, angle }]`, position as canvas fractions and size in px, `angle` in degrees (default 0). Cuts the tail on contact and **fails the level**. Act 3 vocabulary; see **Blades and the fail state** in Key Concepts before placing one, particularly the note that it kills on any contact.
