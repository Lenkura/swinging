...
- **HP system**: `ratHp` starts at 100. Damage per hit = `speed² × angleFactor × material.yoyoDamage × impactMultiplier × comboMultiplier / DAMAGE_SCALE` (`DAMAGE_SCALE = 200`). HP floors at 0; the rat shatters (giblets) when HP reaches 0. `DAMAGE_SCALE` was raised to 2000 in an earlier pass on the (incorrect) assumption that real swings reach ~60% of `pushMaxSpeed`; playtesting showed actual swing speeds land far below that, producing 30-40 hits to clear Level 1, so it was corrected back down to 200 (~3-4 hits on Level 1 for a typical swing). `pushMaxSpeed` is a **presentation knob only** — its single use is normalising the speed meter and the whoosh in `main.js`; it never enters the damage formula. Validated against human free-swing calibration on the rope build (2026-09-12): standard peaks at 720 against its 750, i.e. 96%, so the meter is correctly scaled; heavy peaked at 784 against 625, so heavy was raised to 785. `DAMAGE_SCALE` remains at 200 and has NOT been retuned for the rope — see the 2026-09-12 decision entry for why the data did not support a change yet.
